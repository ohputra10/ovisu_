...
// connect
function wsConnect(req_id:number,username:string,password:string,db?:string){
    send('{"action":"conn","args":{"req_id":0,"user":"root","password":"taosdata","db":"test"}}')
    }

function wsQuery(req_id:number,sql:string){
    send('{"action":"query","args":{"req_id":1,"sql":"select * from test.meters"}}')
}

function fetch(req_id:number,id:string){
    send('{"action":"fetch","args":{"req_id":1,"id":1}}')
}

function fetch_block(req_id:number,id:number){
    send('{"action":"fetch_block","args":{"req_id":1,"id":1}}')
}

function free_result(req_id:number,id:number){
    send('{"action":"free_result","args":{"req_id":1,"id":1}}')
}

function version(){send('{"action":"version"}')}
```
